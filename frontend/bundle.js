...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  var API_ENDPOINT = `${API_BASE_URL}/api`;
  var firebaseConfig = {
  ...
  var currentUserId = localStorage.getItem("growthhub_user_id");
  if (!currentUserId) {
    currentUserId = v4_default();
  ...
  function checkStreak() {
    ...
    if (!lastVisit) {
      ...
      addPoints(25, "Daily visit bonus! \u{1F389}");
      ...
      if (diffDays === 1) {
        ...
        addPoints(25, `${gamificationData.streak} day streak! \u{1F525}`);
        if (gamificationData.streak >= 7 && !gamificationData.badges["streak-master"]) {
          unlockBadge("streak-master", "Streak Master - 7 day streak! \u{1F525}");
        }
      } else if (diffDays > 1) {
        gamificationData.streak = 1;
  ...
  function checkBadges() {
    ...
    if (gamificationData.viewedCount >= 50 && !gamificationData.badges["power-user"]) {
      unlockBadge("power-user", "Power User Badge Unlocked! \u{1F3C6}");
    }
    if (gamificationData.ratedCount >= 5 && !gamificationData.badges["contributor"]) {
      unlockBadge("contributor", "Contributor Badge Unlocked! \u{1F48E}");
    }
  }
  ...
  function completeQuiz() {
    ...
    loadCommunities();
    addPoints(15, "Quiz completed! \u{1F3AF}");
  ...
  function setDailySpotlight(community) {
    ...
    document.getElementById("spotlightMembers").textContent = `\u{1F465} ${formatNumber(community.member_count)} members`;
    document.getElementById("spotlightPlatform").textContent = `\u{1F4F1} ${community.platform}`;
  ...
  function createCommunityCard(community) {
    ...
    card.innerHTML = `
        <button class="save-btn ${isSaved ? "saved" : ""}" data-community-id="${community.id || community.name}">
            <span class="heart">${isSaved ? "\u2764\uFE0F" : "\u{1F90D}"}</span>
        </button>
        
        <div class="card-header">
            <div class="card-image">
                ${community.image_url ? `<img src="${community.image_url}" alt="${community.name}" onerror="this.style.display='none'; this.parentElement.textContent='${platformIcon}'">` : platformIcon}
            </div>
            <div class="card-info">
        ...
        <div class="card-footer">
            ...
            <div class="rating-container">
                <div class="stars" data-community-id="${community.id || community.name}">
                    ${[1, 2, 3, 4, 5].map(
      (star) => `<span class="star ${star <= rating ? "filled" : ""}" data-rating="${star}">\u2605</span>`
    ).join("")}
                </div>
            </div>
        </div>
    ...
    stars.forEach((star) => {
      star.addEventListener("click", (e) => {
        ...
        rateCommunity(community.id || community.name, rating2, card);
      });
    });
    card.addEventListener("click", () => {
      if (community.url || community.invite_link) {
  ...
  function toggleSaveCommunity(communityId, button) {
    ...
    if (index > -1) {
      ...
      button.querySelector(".heart").textContent = "\u{1F90D}";
  ...
  function rateCommunity(communityId, rating, card) {
    const wasRated = gamificationData.ratings[communityId] !== void 0;
  ...
  function trackCommunityView(community) {
    ...
    addPoints(5, "Community viewed! \u{1F441}\uFE0F");
    checkBadges();
    saveGamificationData();
    updateGamificationUI();
  }
  function showToast(type, title, text) {
    ...
    toast.innerHTML = `
        <span class="icon">${icons[type] || "\u{1F4E2}"}</span>
        <div class="message">
